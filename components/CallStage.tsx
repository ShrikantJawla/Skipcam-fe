...
/**
 * 1:1 call stage in the WhatsApp / Zoom / FaceTime pattern:
 * - Before connect: your camera fills the stage (mirrored preview)
 * - Connected: partner fills the stage; you sit in a floating PIP
 * - Both videos stay mounted so MediaStream refs never drop
 * - object-cover fills each tile (no letterbox frames)
 */
export default function CallStage({
  ...
  return (
    <section className="relative min-h-0 flex-1 overflow-hidden rounded-xl bg-black shadow-[0_20px_50px_-24px_rgba(15,23,42,0.55)] sm:rounded-2xl">
      ...
      >
        {/* Partner — always mounted; shown full-bleed when live */}
        <VideoBox
          videoRef={remoteVideoRef}
          fit="cover"
          muted
          className={`absolute inset-0 rounded-none border-0 bg-black transition-opacity duration-300 ${
            connected ? "z-1 opacity-100" : "pointer-events-none z-0 opacity-0"
          }`}
          videoClassName="object-[center_35%]"
          label={connected ? "Stranger" : undefined}
          labelClassName="rounded-full bg-black/55 px-2.5 py-1 text-[11px] font-medium tracking-wide backdrop-blur-sm"
        />

        {/* You — full-bleed preview before connect; PIP while live */}
        {!connected ? (
          <VideoBox
        ...
        {!cameraOn && !connected && (
          <div className="absolute inset-0 z-2 flex items-center justify-center bg-black/80 text-sm font-medium text-white/70">
            Camera off
          </div>
        )}
      </div>
      ...
      {connected && (
        <DraggablePip padding={12}>
          <div className="relative overflow-hidden rounded-2xl bg-black shadow-[0_8px_28px_rgba(0,0,0,0.55)] ring-2 ring-white/35">
            <VideoBox
              videoRef={localVideoRef}
              fit="cover"
              muted
